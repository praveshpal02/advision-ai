...
export default function BrandInputSection() {
  ...
  const [styleWordInput, setStyleWordInput] = useState<string>('');
  ...
  const handleAddStyleWord = () => {
    ...
    const handleStyleWordKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') {
        e.preventDefault();
        handleAddStyleWord();
        }
    ...
    const handleRemoveStyleWord = (word: string) => {
        dispatch({ type: 'REMOVE_STYLE_WORD', payload: word });
    };


  const handleTargetAudienceChange = (e: ChangeEvent<HTMLTextAreaElement>) => {
